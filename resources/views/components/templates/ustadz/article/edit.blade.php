...
@section('mainContent')
    ...
    <div class="min-h-screen w-full py-4 px-7">
        <span class="font-bold text-lg md:text-2xl text-black">Edit Artikel</span>
            ...
            <div class="block w-full p-6 bg-[#EEEBDD] border border-black rounded-lg shadow">
                <form method="POST" action="{{ route($role . '.article.store') }}">
                    ...
                    <a href="{{ url()->previous() }}">
                        ...
                        <button type="edit"
                            class="w-28 text-white bg-[#1F7632] hover:bg-[#1F7632] focus:outline-none focus:ring-4 focus:ring-green-300 font-bold rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-[#1F7632] dark:hover:bg-[#1F7632] dark:focus:ring-[#1F7632]">EDIT</button>
                </form>
            </div>
        </div>
    </div>
